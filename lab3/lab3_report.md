...
# Lab3. Эмуляция распределённой корпоративной сети связи, настройка OSPF и MPLS, организация первого EoMPLS
...
В рамках лабораторной работы требовалось развернуть в ContainerLab распределённую IP/MPLS-сеть связи, объединяющую несколько офисов.  
...
В ходе выполнения лабораторной работы была успешно развернута распределённая корпоративная сеть. На маршрутизаторах была выполнена настройка IP-адресации, протокола динамической маршрутизации OSPF, MPLS и LDP. Между удалёнными узлами `R01.NY` и `R01.SPB` был организован EoMPLS (VPLS), что позволило объединить контейнеры `SGI Prism` и `PC1` в единый L2-сегмент.
